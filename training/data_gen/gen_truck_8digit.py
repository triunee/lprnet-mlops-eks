# num_img = 70 | 예상 생성량: 21 × 70 = 1,470장 → result32/
import os
import random
import uuid
...
class ImageGenerator:
    def __init__(self, save_dir, plates_dir, nums_dir, chars_dir):
        ...
        # load number - 수정된 부분: 숫자 파일을 순서대로 정렬하여 로드
        self.numbers = []
        self.number_list = []
        file_list = sorted(os.listdir(nums_dir), key=lambda x: int(x.split('.')[0]))
        for filename in file_list:
            img_path = os.path.join(nums_dir, filename)
        ...
        self.char_mapping = {
            ...
            "chung": "충", "in": "인", "jeon": "전",
            # 추가된 매핑
            "yuk": "육", "hae": "해", "hap": "합", "guk": "국", "gong": "공"
        }
    ...
    def type_a(self, num):
        ...
        for p in os.listdir(self.plates_dir):
            ...
            numbers = [cv2.resize(number, number_size) for number in self.numbers if number is not None]
            # 유효한 이미지만 사용
            valid_chars = [char for char in self.chars if char is not None]
            chars = [cv2.resize(char, char_size) for char in valid_chars]
            ...
            for i in range(num):
                ...
                with open(f"{self.save_dir}/labels/{unique_id}.txt", "w") as f:
                    ...
                    for idx, width in enumerate(widths):
                        if idx < digit_count:
                            # 앞 숫자 (화물차량용 800-979 범위로 제한)
                            if idx == 0:
                                # 첫 번째 숫자는 8 또는 9만 가능
                                first_digit_idx = random.choice([8, 9])
                                first_digit = str(first_digit_idx)
                                rand_int = self.number_list.index(first_digit)
                            elif idx == 1:
                                # 두 번째 숫자: 첫 번째가 9면 0-7, 8이면 0-9
                                if first_digit == '9':
                                    second_digit_idx = random.randint(0, 7)
                                else:  # first_digit == '8'
                                    second_digit_idx = random.randint(0, 9)
                                second_digit = str(second_digit_idx)
                                rand_int = self.number_list.index(second_digit)
                            else:
                                # 세 번째 숫자는 랜덤
                            ...
                            plate_content.append(self.number_list[rand_int])
    ...
    def type_b(self, num):
        ...
        numbers = [cv2.resize(number, (45, 83)) for number in self.numbers if number is not None]
        valid_chars = [char for char in self.chars if char is not None]
        chars = [cv2.resize(char, (49, 70)) for char in valid_chars]
        ...
        for p in os.listdir(self.plates_dir):
            ...
            for i in range(num):
                ...
                with open(label_path, 'w') as f:
                    ...
                    first_digit_idx = random.choice([8, 9])
                    first_digit = str(first_digit_idx)
                    first_digit_pos = self.number_list.index(first_digit)
                    
                    patch = Plate[row:row + 83, col:col + 45, :]
                    Plate[row:row + 83, col:col + 45, :] = self.add(patch, self.random_bright(numbers[first_digit_pos]))

                    x2, y2 = x1 + 45, y1 + 83
                    x_center_norm, y_center_norm = (x1 + x2) / (2 * 355), (y1 + y2) / (2 * 155)
                    width_norm, height_norm = (45 / 355), (83 / 155)

                    cls_idx = names.index(self.number_list[first_digit_pos])
                    f.write(f'{cls_idx} {x_center_norm} {y_center_norm} {width_norm} {height_norm}\n')
                    plate_content.append(first_digit)

                    col += 45
                    x1, y1 = x2, y1
                    
                    # number 2 - 두 번째 숫자 (첫 번째가 9면 0-7, 8이면 0-9)
                    if first_digit == '9':
                        second_digit_idx = random.randint(0, 7)
                    else:  # first_digit == '8'
                        second_digit_idx = random.randint(0, 9)
                    ...
                    second_digit = str(second_digit_idx)
                    second_digit_pos = self.number_list.index(second_digit)
                    
                    patch = Plate[row:row + 83, col:col + 45, :]
                    Plate[row:row + 83, col:col + 45, :] = self.add(patch, self.random_bright(numbers[second_digit_pos]))

                    x2, y2 = x1 + 45, y1 + 83
                    x_center_norm, y_center_norm = (x1 + x2) / (2 * 355), (y1 + y2) / (2 * 155)
                    width_norm, height_norm = (45 / 355), (83 / 155)

                    cls_idx = names.index(self.number_list[second_digit_pos])
                    f.write(f'{cls_idx} {x_center_norm} {y_center_norm} {width_norm} {height_norm}\n')
                    plate_content.append(second_digit)
                    ...
                    # numbers 4-7 (뒷자리 네 개 숫자)
                    for j in range(4):
                        rand_int = random.randint(0, 9)
                        patch_col = col if j == 0 else col  # 첫 번째 숫자는 위치 조정
                        
                        Plate[row:row + 83, patch_col:patch_col + 45, :] = self.add(
                            Plate[row:row + 83, patch_col:patch_col + 45, :],
...
if __name__ == '__main__':
    ...
    if not os.path.exists('./result32'):
        os.mkdir('./result32')
    if not os.path.exists('./result32/images'):
        os.mkdir('./result32/images')
    if not os.path.exists('./result32/labels'):
        os.mkdir('./result32/labels')

    Type_A1 = ImageGenerator(save_dir='./result32/',
                             plates_dir='./assets/plates/type_a',
                             nums_dir='./assets/nums/',
                             chars_dir='./assets/chars_truck/')

    # 오직 type_a만 실행하도록 수정
    num_img = 70
    generated_count = Type_A1.type_a(num_img)
    print(f"Type A 완료: {generated_count}장 생성됨")